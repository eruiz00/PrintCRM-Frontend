// MD5 puro JS — implementación clásica de Paul Johnston (dominio público).
// Se expone como `md5(text)` -> string hexadecimal de 32 caracteres
// (minúsculas).  Se usa para hashear la contraseña del empleado antes
// de mandarla al backend.
...
const cmn = (
    ...
    s: number,
    t: number
): number => add32(rol(add32(add32(a, q), add32(x, t)), s), b);
...
const md51 = (input: string): number[] => {
    // Convierte UTF-16 -> bytes UTF-8 (en formato de char 0-255).
